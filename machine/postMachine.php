...
if (!isset($data['name']) || !isset($data['device_idDevice'])) {
    http_response_code(400); 
    $errorMessage = "Machinename and device_idDevice are required.";
...
$d_entry_startstop = $machineconn->real_escape_string(trim($data['d_entry_startstop'] ?? null));
$d_entry_counter = $machineconn->real_escape_string(trim($data['d_entry_counter'] ?? null));
$device_idDevice = $machineconn->real_escape_string(trim($data['device_idDevice']));

$checkSql = "SELECT * FROM machine WHERE name = '$name'";
